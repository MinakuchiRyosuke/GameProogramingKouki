...
//コンスタントバッファの作成
[[nodiscard]] bool ConstantBuffer::create(const Dx12& dx12, const DescriptorHeap& heap, UINT bufferSize, UINT descriptorIndex) noexcept {
	//アライメント済みサイズの計算
    ...
    const auto res = dx12.getDevice()->CreateCommittedResource(
        &heapProps,
    ...
    if (heapType != D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) {
        assert(false && "ディスクリプタヒープのタイプがCBV_SRV_UAVではありません");
        false;
    }
    ...
    // ディスクリプタのサイズを取得
    UINT cbvDescriptorSize = dx12.getDevice()->GetDescriptorHandleIncrementSize(heap.getType());
    ...
    dx12.getDevice()->CreateConstantBufferView(&cbvDesc, cpuHandle);
